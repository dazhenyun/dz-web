...
export default ({ typeText = "组件负责人", title, account = "someBody", description, mb = 20, assist, link = {}, extra = {} }) => {
	const { url, type } = link;
	const { github, gitlab, npm, maturity, useList = [] } = extra;

	const getUser = (uid) => {
		let userItem = uedPartners.find(user => user.account === uid);
		return userItem ? userItem : uedPartners.find(user => user.account === "someBody");
	};
	...
	return (
		<div className="author-component" style={{ marginBottom: mb || 0 }}>
			...
			<div className='author-body' style={{ borderLeftColor: authorInfo.color }}>
				<div className='avatar'>
					...
					<TextAvatar
						...
						onClick={() => {
							window.open(`https://sinan.tongdun.me/user/${authorInfo.sinanUid}`);
						}}
						theme='plant'
					>
						<img
				...
				<div className='info'>
					<h4>
						{typeText}：{authorInfo.name}
						{
							assist && <span>、{getUser(assist).name}</span>
						}
					</h4>
					<div className='extra'>
						{

...
class Hooks extends PureComponent {
	...
	state = {
		...
		defaultOpenKeys: ["state", "ui", "sideEffect", "dom", "advanced"]
	};
	...
	componentDidMount() {
		...
		// 判断url是否是正确的页面
		const urlIsExist = getMenuNameList(menuList, name).find(item => item === name);
